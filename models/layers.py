...
def ncsn_conv1x1(x, out_planes, stride=1, bias=True, dilation=1, init_scale=1.0):
    ...
    output = nn.Conv(out_planes, kernel_size=(1, 1), strides=(stride, stride), padding="SAME", use_bias=bias, kernel_dilation=(dilation, dilation), kernel_init=kernel_init, bias_init=bias_init)(x)
    return output
...
def ddpm_conv1x1(x, out_planes, stride=1, bias=True, dilation=1, init_scale=1.0):
    ...
    output = nn.Conv(
        out_planes, kernel_size=(1, 1), strides=(stride, stride), padding="SAME", use_bias=bias, kernel_dilation=(dilation, dilation), kernel_init=default_init(init_scale), bias_init=bias_init
    )(x)
    return output
...
def ncsn_conv3x3(x, out_planes, stride=1, bias=True, dilation=1, init_scale=1.0):
    ...
    bias_init = lambda key, shape: kernel_init(key, kernel_shape)[0, 0, 0, :]
    output = nn.Conv(out_planes, kernel_size=(3, 3), strides=(stride, stride), padding="SAME", use_bias=bias, kernel_dilation=(dilation, dilation), kernel_init=kernel_init, bias_init=bias_init)(x)
    return output
...
def ddpm_conv3x3(x, out_planes, stride=1, bias=True, dilation=1, init_scale=1.0):
    ...
    bias_init = jnn.initializers.zeros
    output = nn.Conv(
        out_planes, kernel_size=(3, 3), strides=(stride, stride), padding="SAME", use_bias=bias, kernel_dilation=(dilation, dilation), kernel_init=default_init(init_scale), bias_init=bias_init
    )(x)
    return output
...
class RefineBlock(nn.Module):
    ...
    def __call__(self, xs):
        ...
        rcu_block_output = functools.partial(RCUBlock, features=self.features, n_blocks=3 if self.end else 1, n_stages=2, act=self.act)
        hs = []
        for i in range(len(xs)):
...
class CondRefineBlock(nn.Module):
    ...
    def __call__(self, xs, y):
        ...
        rcu_block_output = functools.partial(CondRCUBlock, features=self.features, n_blocks=3 if self.end else 1, n_stages=2, act=self.act, normalizer=self.normalizer)
        hs = []
        for i in range(len(xs)):
        ...
        if not self.start:
            msf = functools.partial(CondMSFBlock, features=self.features, interpolation=self.interpolation, normalizer=self.normalizer)
            h = msf(shape=self.output_shape)(hs, y)
        else:
            h = hs[0]

        crp = functools.partial(CondCRPBlock, features=self.features, n_stages=2, act=self.act, normalizer=self.normalizer)
        h = crp()(h, y)
...
class ConvMeanPool(nn.Module):
    ...
    def __call__(self, inputs):
        output = nn.Conv(features=self.output_dim, kernel_size=(self.kernel_size, self.kernel_size), strides=(1, 1), padding="SAME", use_bias=self.biases)(inputs)
...
class MeanPoolConv(nn.Module):
    ...
    def __call__(self, inputs):
        output = inputs
        output = sum([output[:, ::2, ::2, :], output[:, 1::2, ::2, :], output[:, ::2, 1::2, :], output[:, 1::2, 1::2, :]]) / 4.0
        output = nn.Conv(features=self.output_dim, kernel_size=(self.kernel_size, self.kernel_size), strides=(1, 1), padding="SAME", use_bias=self.biases)(output)
        return output
